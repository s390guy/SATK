...
import literal       # Access liteal pool
...
#    seq        Enable 80 column card handling with True
class FileSource(InputSource):
    def __init__(self,typ,filename,stmtno=None,srcno=None,seq=False):
        ...
        self.seq=seq              # True if 80 column card handling enabled
    ...
    def getLine(self,debug=False):
        ...
        return StreamLine(Source(lineno=self.lineno,fileno=self.fileno),\
            line,seq=self.seq)
...
class InjectableSource(InputSource):
   ...
   #
    ...
    # Returns an injected PhysLine object when called
    # Exception:
    #   SourceEmpty when injected lines exhausted
    def getLine(self,debug=False):
        try:
            line=self.lines[0]
            del self.lines[0]
        ...
        #print("%s input line: %s" \
        #    % (assembler.eloc(self,"getLine",module=this_module),line))
        return line
   ...
   #
    ...
    # Inject into the input stream a physical line
...
class MacroSource(InputSource):
    def __init__(self,typ,exp,stmtno=None,srcno=None,seq=False):
    ...
    # Returns a macro generated logical line from a macro model statement
    # Returns:
...
class PoolSource(InjectableSource):
    spaces=" "*15
    def __init__(self,pool,genlvl=None,debug=False):
        assert isinstance(pool,literal.LiteralPool),\
            "%s 'pool' argument must be a literal.LiteralPool object: %s" \
                % (assembler.eloc(self,"__init__",module=this_module),pool)

        sid="[%s]Pool-%s" % (pool.ltorg,pool.pool_id)       
        super().__init__("P",sid)
        self.pool=pool       # literal.LiteralPool being created
        self.genlvl=genlvl   # This is the gen level of the LTORG creating this pool
        
        spaces=PoolSource.spaces
        for lit in pool.pool_list:
            string="%s%s" % (spaces,lit.name)
            pline=StreamLine(None,string,genlvl=genlvl)
            pline.literal=lit
            self.inject(pline,debug=debug)
...
class StreamLine(PhysLine):
    def __init__(self,source,content,genlvl=None,seq=False):
        self.seq=seq         # Enable 80 column card handling
        super().__init__(source,content,genlvl=genlvl)

    def _column_80(self):
        # Actually have an 80 column text record
        # First ignore the sequence columns
    ...
    def init(self):
        ...
        if self.seq and len(self.content) == 80:
            self._column_80()
        else:
            self._variable()
...
class LineBuffer(object):
    ...
    def __init__(self,depth=20,env="ASMPATH",pathmgr=None,seq=False):
    ...
    # Adds a source to nest of input sources and makes it the current input source
    ...
    def __source(self,typ,sid,stmtno=None,srcno=0):
        ...
        srco=src_cls(typ,sid,srcno=srcno,seq=self._seq)
        try:
    ...
    # Initiate a new literal pool
    def newPool(self,pool,genlvl=None,debug=False):
        poolstmts=PoolSource(pool,genlvl=genlvl,debug=debug)
        self.__appendSource(poolstmts)
