...
*
         TITLE 'PGM5 - HELLO WORLD WITH BOOT LOADER SERVICES'
* Program Description:
*
* PGM5 is a bare-metal 'Hello World' program.  It uses a boot loader and its
* services to issue the message.  The program is executed by means of a FBA
* DASD boot loader configured for S/360 operation and boot loader services.
*
* Target Architecture: S/360
*
* Devices Used:
*   00F - Console device (used by this program)
*   110 - FBA IPL volume (used by the boot loader)
*
...
*
* Other than R15, upon entry to the booted program, the program should assume
* that the other registers' content is unpredictable, namely R0-R14.
*
...
*                address X'8'
*    X'000018' - Unexpected External interruption occurred.  Old External PSW
*                at address X'18'
...
*    X'05100C' - Console Device X'00F' or channel had a problem. See SCSW.
         EJECT
...
* This results in BOOTED5.bin being created in the list directed IPL directory
         USING ASA,0      Give me instruction access to the ASA and LOD1 CSECT
...
* Enable the console device for operation
         ...
         BLSCALL SPB=ENASPB
         B     *+4(15)   Use branch table to analyze return code...
...
ENDBAD   DWAIT PGM=05,CMP=1,CODE=000  Updated with abend code
         TITLE 'PGM5 - BOOT LOADER SERVICES STRUCTURES'
*
...
ENASPB   DC    Y(ENADEV)    Service ID for ENADEV service (from BLSTABLE)
         ...
         SPACE 1
* ENADEV Service Parameter Block Extension
...
ENASPBC  DC    AL1(BLSCON)  Input - Enabling a console device (from BLIOT)
         DC    XL3'00'
         SPACE 3
* EXCP Service Parameter Block (See BLS.odt or BLS.pdf for details)
         SPACE 1
         DS    0F
EXCPSPB  DC    Y(EXCP)      Service ID for EXCP service (from BLSTABLE)
         ...
         DC    XL1'00'      not used
         SPACE 1
* EXCP Service Parameter Block Extension
...
*
         SPACE 1
* This DSECT allows symbolic access to these assigned locations.  The DSECT
* created is named ASA and contains the assigned storage locations and boot
* loader LOD1 record definition.
ASA      BLSASA
         END