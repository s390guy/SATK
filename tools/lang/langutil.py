...
from satkutil import DM   # Access the debug manager if needed
...
class KWLang(lang.Processor):
    ...
    def __init_lexer(self,kdebug=False,tdebug=False):
        ...
        self.token(lexer.EmptyType(debug=tdebug))
    ...
    def __find_token_errors(self,eo):
        if self.lang.isdebug("edebug"):
            print("langutil.py - KWLang.__find_token_errors eo: %s" % eo)
        lst=[]
        for x in eo:
    ...
    def __recognize_new_token_errors(self,te,eo):
        ...
        if self.lang.isdebug("edebug"):
            print("langutil.py - KWLang.__recognize_new_token_error, y lst=%s" \
                % lst)
            print("langutil.py - KWLang.__recognize_new_token_error, x te=%s" % te)
        if len(te)==0:
            add=lst
        else:
            add=[]
    ...
    def cb_argument_end(self,gs,pid,failed=False,eo=[]):
        ...
        if failed:
            ...
            #    who="'%s' statement argument '%s' value" \
            #        % (gs.stmt_def.name,gs.keyword)
            self.__token_errors(who,gs.argument_eos)
    ...
    def cb_positional_end(self,gs,pid,failed=False,eo=[]):
        ...
        if failed:
            for x in gs.positional_eos:
                gs.mgr.report(x)
            self.semerror("cb_positional_end",token=gs.statement_token,\
    ...
    def cb_statement_end(self,gs,pid,failed=False,eo=[]):
        ...
        if len(required)!=0:
            ...
            stmt=kwdef.name
            for x in required:
                self.semerror("cb_statement_end",token=stmt_tok,\
    ...
    def init_lang(self,dm=None):
        if dm is None:
            self.dm=DM(parser=True,langutil=True,lexer=True)
        else:
            self.dm=dm
